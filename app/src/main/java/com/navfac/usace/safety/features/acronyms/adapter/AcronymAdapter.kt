package com.navfac.usace.safety.features.acronyms.adapter
...
class AcronymAdapter(
    ...
    private var acronymnlist: List<AcronymModelItem>,
    private val realmBookmark: Realm
) : RecyclerView.Adapter<AcronymAdapter.Holder>() {

    internal var actionListener: (model: AcronymModelItem, imageView: ImageView) -> Unit =
        { _, _ -> }
    ...
    override fun onBindViewHolder(holder: Holder, position: Int) {
        ...
        holder.menuicon.apply {
            val exists =
                realmBookmark.query<BookmarkModel>("name == $0", acronymnlist[position].name)
                    .first()
                    .find() != null
