package com.navfac.usace.safety.features.formfigure.form
...
import com.navfac.usace.safety.databinding.FragmentFormBinding
...
class FormFragment : BaseFragment<FragmentFormBinding>() {
    ...
    override fun onCreated(savedInstance: Bundle?) {
        val gson = Gson()
        //model = gson.fromJson(BaseLocalJsonParser.parseJSONData(requireContext(), "forms/forms.json"), FigureFormModel::class.java)
        model = gson.fromJson("forms/forms.json".toJson(requireContext()) ?: "")
            ...
            builder.setItems(options) { dialog: DialogInterface, item: Int ->
                if (options[item] == "Add to Bookmark") {
                    ...
                    if (bookmark != null) {
                        ...
                        dialog.dismiss()
                        bookMarkIcon.setImageDrawable(ResourcesCompat.getDrawable(resources, R.drawable.ic_bookmark_red_asset, null))
                    }
